...
?>
<div ng-show="showTomorrowSlot"
     id="tomorrow-booking-area">
    <h4 class="tac mt20 mb20">
        <?php echo App::t("frontend.shop.title", "{tomorrow-date}（{tomorrow-day-of-week})", [
            "tomorrow-date" => App::$app->formatter->asDate(time() + 1 * 24 * 60 * 60, "d/M/y"),
            "tomorrow-day-of-week" => App::$app->formatter->asDayOfWeek(time() + 1 * 24 * 60 * 60),
        ]) ?>
    </h4>
    <?php foreach ($workers as $worker) { ?>
        <div id="container_worker_<?php echo $model->dateTomorrow ?>_<?php echo $worker['worker_id'] ?>"
             ng-hide="(slotManager.slotsData['<?php echo $model->dateTomorrow ?>'].slots[<?php echo $worker['worker_id'] ?>]|filter: slotManager.filterCanBooking).length <= 0"
            ...
            <div class="panel panel-default">
                <div class="panel-heading box-container box-space-between box-middle">
                    ...
                    <div class="box-container box-middle">
                        ...
                        <a data-toggle="collapse"
                           href="#worker_<?php echo $model->dateTomorrow ?>_<?php echo $worker['worker_id'] ?>"
                ...
                <div id="worker_<?php echo $model->dateTomorrow ?>_<?php echo $worker['worker_id'] ?>"
                     class="panel-collapse collapse <?php echo $worker_id == $worker['worker_id'] ? "in" : ""; ?>">
                    <div class="panel-body">
                        <div class="box-container box-space-between mb10"
                             ng-repeat="slot in slotManager.slotsData['<?php echo $model->dateTomorrow ?>'].slots[<?php echo $worker['worker_id'] ?>] | filter:convertTime"
